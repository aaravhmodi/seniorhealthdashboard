...
| NHAMCS loader | **fixed, verified on real 2018–2022 files** (branch `worktree-integrate-datasets`) | `datasets/nhamcs.py` |
...
**No real dataset is in the repo's warehouse yet**, so a fresh checkout still
serves MOCK cards. Real NHAMCS has been loaded and checked in a scratch
warehouse only (see "Dataset integration session" below).
...
## Dataset integration session (2026-09-19, branch `worktree-integrate-datasets`)

Work lives in the worktree `.claude/worktrees/integrate-datasets` on branch
`worktree-integrate-datasets`, based on `de75662`. **Not merged to `main` and
not pushed yet.**

### Done (commit `eb1248d`, "NHAMCS: read the files the CDC actually ships")
- `nhamcs.py` reads the real public-use layout. It prefers `RFV13D`, normalises
  a 5-digit `RFV1` by dividing by 10, and takes the outcome from the disposition
  flags. It falls back to a coded `DISPO`/`ADISP` column for old-style CSVs.
- `nhamcs.load()` takes `.dta` directly. `stata_to_csv()` converts each file
  once to `*.slim.csv` beside it, keeping about 25 columns (rate inputs plus
  triage vitals for the model). pandas is in the new
  `backend/requirements-data.txt`, not in `requirements.txt`, so the Docker
  image stays slim.
- **Rates are aggregated by symptom, not by code.** The old version filtered
  n≥30 per code and then summed, so multi-code symptoms (urinary is 5 codes)
  vanished and the CI was a min/max stitch. The table is now
  `nhamcs_senior_rates(symptom, age_band, n, weighted_n, rate, ci_low, ci_high)`.
  Each symptom also gets an all-seniors `age_band='65+'` row, which
  `lookup.admission_rate` uses when the patient's band is too thin.
- `SYMPTOM_TO_RFV` was rebuilt against the value labels inside the Stata files
  and the 2022 codebook PDF. `fall` was removed: the classification has no fall
  code, so NEISS answers falls.
- New tests: the real layout, 5-digit `RFV1`-only files, `.dta` round trip
  (skips without pandas), every lexicon symptom except fall has a code, and no
  code maps to two symptoms. Suite: 195 passed, 5 skipped.

### Second commit (with this handover): confusion code + FAERS file discovery
- `nhamcs.py`: `confusion` now maps to `1240, 5842, 1215`. The 2022 codebook
  (p.116) lists "Confusion, Cognitive decline" under 1240. It also takes in
  pinched nerves and neuralgia; that noise is accepted and commented.
- `faers.py` `_quarter_tables`: accepts a quarter folder *or* its `ASCII/`
  subfolder, and matches `DEMO/DRUG/REAC*.txt` case-insensitively, once per
  file (the old double glob read every row twice on Windows). **Needs a test**
  (quarter inside `ASCII/`, lower-case filenames) before committing.

### Verified on real data (scratch warehouse, not committed)
NHAMCS ED 2018–2022, 91k visits, 16k aged 65+: 12 of 14 symptoms have real
rates. Share not sent home, by age band 65–74 / 75–84 / 85+:

| symptom | 65–74 | 75–84 | 85+ |
|---|---|---|---|
| shortness of breath | 50% (n=501) | 58% | 68% |
| chest pain | 37% (n=429) | 44% | 46% |
| confusion | 60% (n=36) | 57% | 54% |
| weakness one side | 64% (all 65+, n=49) | | |
| dizziness | 25% | 26% | 39% |
| back pain | 12% | 16% | 23% |
| urinary | 13% | 13% | 23% |

Still below n=30: poor appetite (1570) and trouble sleeping (1135). They show
no NHAMCS card, which is correct.

To reproduce, download the Stata zips from
`https://ftp.cdc.gov/pub/Health_Statistics/NCHS/dataset_documentation/NHAMCS/stata/`
(`ED2018-stata.zip` … `ed2022-stata.zip`, about 5 MB each), unzip them into
`backend/data/raw/nhamcs/`, then run
`pip install -r requirements-data.txt && python -m app.datasets.cli nhamcs "data/raw/nhamcs/*.dta"`.
The session's copies are in the job temp dir and are deleted with the job.

### Next steps, in order
1. Add a test for the FAERS `ASCII/` + case fix. Load FAERS 2024q3/q4
   (`https://fis.fda.gov/content/Exports/faers_ascii_2024q3.zip`, downloads
   fine by curl) and check the positive control: warfarin → bleeding significant.
2. NEISS: download one year in a browser, load it, and check that "I fell down
   the stairs and hit my head" for `sen_chen` gives non-MOCK fall cards.
3. **Wire similar cases into the evaluation.** `retrieval.similar_cases()` is
   only reachable at `GET /evidence/similar-cases`, so the ladder, explanation
   and ER handoff never see it. Add it as a NEISS card for fall/injury
   check-ins with `weight=0`, so it informs without double-counting the cohort
   rate in the ladder. Only add it when `cases_represented` is large enough.
4. **Don't mix MOCK with real.** With the warehouse loaded, a fall check-in
   still gets the MOCK `_NEISS_MOCK["fall"]` outcome card next to the real
   NEISS fall card, because NHAMCS has no fall code. Skip a mock card when a
   real card for the same symptom exists.
5. Add a `cli fetch` command (NHAMCS Stata + FAERS by URL, printed
   instructions for NEISS). Update `docs/DATA.md` (it still says NHAMCS CSV and
   `nhamcs/*.csv`), `ENGINE_VERSION` in `ladder.py` (`...+mockevidence`) and the
   "placeholders" line in the `main.py` docstring.

## Gotchas that were checked, not guessed

- **NHAMCS: skip the fixed-width file, use the Stata one.** `ed2022.zip` on
  the FTP root is fixed-width, but the CDC also ships
  `dataset_documentation/NHAMCS/stata/ed20XX-stata.zip` (2018–2022), with the
  headers and the codebook value labels inside. `nhamcs.load()` now takes the
  `.dta` directly. No fixed-width reader is needed.
- **NHAMCS `RFV1` is 5 digits** (14150); the classification and our map are
  4-digit (`RFV13D`, 1415). **There is no `DISPO`**: the outcome is 0/1 flags
  `ADMITHOS/TRANOTH/TRANPSYC/OBSHOS/DIEDED`. `ADISP` is where a patient went
  and is -7 for most rows. All three were silent failures: MOCK forever, no error.
- **CPSC blocks scripted NEISS downloads** (403 to curl, even with browser
  headers). Download `neissYYYY.xlsx/.tsv` in a browser from the NEISS page.
- **FAERS zips unpack into `ASCII/`**, and on Windows `glob("DEMO*") +
  glob("demo*")` matches the same file twice. Both are handled in `faers.py`
  (committed, but not yet run on a real quarter).
...
2. Wrote the NHAMCS loader for an imagined CSV. On the real files it matched
   nothing, and the symptom→code map was mostly wrong: `fall` pointed at 5820,
   which is **suicide attempt**. Fixed by reading the codebook labels.
